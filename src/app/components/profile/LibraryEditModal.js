...
class LibraryEditModal extends PureComponent {
  ...
  fetchHandler = R.curry((id, params) => {
    if (!this.locals.isFetching) {
      this.locals.isFetching = true;
      this.props.fetchLibrary(id, params)
        .then(() => {
          this.locals.isFetching = false;
        })
        .catch(() => { this.locals.isFetching = false; });
    }
  })
  ...
  renderTopBooks = () => {
    const { profilePage: { topBooks } } = this.props;
    if (topBooks) {
      return (
